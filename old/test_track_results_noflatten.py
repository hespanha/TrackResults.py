...
class TestTrackResults(unittest.TestCase):
    ...
    def test_add(self):
        ...
        self.tracker.remove(filter={"results.acc": 1.0}, simulate=True, silent=True)
        df = self.tracker.get()
        self.assertEqual(len(df), 3)
        self.tracker.remove(filter={"results.acc": 1.0}, simulate=False, silent=True)
        df = self.tracker.get()
        self.assertEqual(len(df), 3)

        # 1 record matches query
        self.tracker.remove(filter={"results.acc": 0.9}, simulate=True, silent=True)
        df = self.tracker.get()
        self.assertEqual(len(df), 3)
        self.tracker.remove(filter={"results.acc": 0.9}, simulate=False, silent=True)
        df = self.tracker.get()
